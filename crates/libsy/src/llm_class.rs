...
//! interfaces this is just two ordinary `target.call`s inside one
//! `process_request`: first the classifier target (to get a score), then the
...
/// Builder that wires a target set into an [`LlmClassifierOrchAlgo`].
///
/// The target set must contain targets named `classifier_model`, `strong_model`,
/// and `weak_model`.
pub struct LlmClassifierOrchAlgoBuilder {
    classifier_model: String,
    strong_model: String,
    weak_model: String,
    threshold: f64,
...
impl LlmClassifierOrchAlgoBuilder {
    /// Configure the classifier: the model that scores each request, the strong
    /// and weak models to route to, and the score `threshold` at or above which
    /// the strong model is chosen.
    pub fn new(
