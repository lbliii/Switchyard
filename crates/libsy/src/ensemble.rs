...
impl EnsembleOrchAlgo {
    ...
    /// candidate (`0` = never commit, ensemble every request). Usually built via
    /// [`EnsembleOrchAlgoBuilder`].
    pub fn new(
        candidate_models: Vec<String>,
...
/// Builder that wires a target set into an [`EnsembleOrchAlgo`].
///
/// The target set must contain a target for every candidate model and for the
/// judge model.
pub struct EnsembleOrchAlgoBuilder {
    candidate_models: Vec<String>,
    judge_model: String,
    exploration_turns: u64,
...
impl EnsembleOrchAlgoBuilder {
    /// Configure the ensemble: the candidate models to fan out to, the judge
    /// model that picks the best response, and how many exploration turns to run
    /// before committing to the winningest candidate (`0` = never commit).
    pub fn new(
        candidate_models: Vec<String>,
        judge_model: impl Into<String>,
        exploration_turns: u64,
